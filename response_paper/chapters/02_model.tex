...
For a specified emergency state, the regulator first reports the outcome vector
\begin{equation}\label{eq:regulator-outcome-vector}
 \mathcal O^\omega(\Pi)
 :=\bigl(J_{\mathrm{TSTT}}^\omega(\Pi),
 J_{\mathrm{order}}^\omega(\Pi),
 J_{\mathrm{risk}}^\omega(\Pi),
 J_{\mathrm{critical}}^\omega(\Pi),
 C_{\mathrm{gov}}(\Pi,\gamma)\bigr).
\end{equation}
When a single arrangement must be selected, we use an epsilon-constrained rule: minimize TSTT plus governance burden subject to declared upper bounds on commercial-order delay and critical-edge overload. Weighted sums are secondary sensitivity analyses. This separation prevents a reported policy ranking from being determined solely by arbitrary weights when order delay is already part of TSTT or when risk and critical-edge indicators are correlated.
